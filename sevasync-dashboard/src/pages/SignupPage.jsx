  ...
  if (success) {
    ...
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 relative overflow-hidden font-sans">
        <div className={`absolute inset-0 bg-gradient-to-br ${isAdminPending ? 'from-amber-950/20' : 'from-emerald-950/20'} to-slate-950`} />
        <div className="text-center space-y-8 max-w-sm relative z-10">
          <motion.div 
             initial={{ scale: 0 }}
             animate={{ scale: 1 }}
             className={`w-24 h-24 ${isAdminPending ? 'bg-amber-500/20 border-amber-500/30' : 'bg-emerald-500/20 border-emerald-500/30'} rounded-full flex items-center justify-center mx-auto mb-6 border`}
          >
            {isAdminPending ? (
          ...
          <h2 className="text-[32px] font-black text-white uppercase tracking-tighter">
            {isAdminPending ? 'Approval Requested' : 'Agent Registered'}
          </h2>
          <p className="text-slate-400 text-[14px] font-medium uppercase tracking-widest leading-relaxed">
             {isAdminPending 
               ? 'YOUR ADMIN REGISTRATION IS REGISTERED. AWAITING COMMANDER APPROVAL BEFORE LOGIN ACTIVATION...' 
          ...
          <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden border border-white/5">
            <motion.div 
               ...
               className={`h-full ${isAdminPending ? 'bg-amber-500 shadow-[0_0_10px_#f59e0b]' : 'bg-emerald-500 shadow-[0_0_10px_#10b981]'}`}
            />
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
               ...
               <div className="grid grid-cols-2 gap-3">
                  {[
                    { val: 'Volunteer', label: 'Volunteer Dashboard', icon: Radio },
                    { val: 'admin', label: 'Admin Panel', icon: UserCog },
                  ].map((r) => (
                    <button
                      key={r.val}
                      type="button"
